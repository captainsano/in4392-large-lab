...
and provisioner, affected by the factors, task arrival time and those listed in \autoref{table:experiment_param_setup}.
...
virtual private cloud settings provided by AWS. A custom security group is used for exposing ports 8000 (master),
 ...
 the audit trail report. We run the workload on three variations of system policies by adjusting the parameters,
 firstly \textsc{C3}, which represents the baseline setup (current) of WantCloud BV, secondly \textsc{E0} -- elastic 0-15 VMs
 with threshold set to 0 and thirdly, \textsc{E7} -- elastic 0-15 VMs with threshold set to 7.

 \begin{table}[htbp]
  \centering
  \caption{System parameter (factors) setup for experiments}
  \label{table:experiment_param_setup}
  \begin{tabular}{|c|c|c|c|}
    \hline
    & MIN\_VMS & MAX\_VMS & THRESHOLD \\
    \hline
    C3 & 3 & 3 & - \\
    E0 & 0 & 15 & 0 \\
    E7 & 0 & 15 & 7 \\
